...
function getSong(req , res) {
	...
	Song.findById(songId).populate({path: 'album'}).exec((err,song) => {
		if (err) {
			res.status(500).send({message : 'getSong error song'});
		}else{
			if (!song) {
...
function saveSong(req, res) {
	...
	song.save((err , songStored) =>{
		if(err){
			res.status(500).send({message : 'no se guardo error song'});
		}else{
			if (!songStored) {
				res.status(404).send({message : 'no se ha cuagdado el song'});
			}else{
...
function getSongs(req, res){
	...
	find.populate({path:'album', populate:{ path:'artist', model: 'Artist'}}).exec((err, songs) => {
		if (err) {
			...
			if (!songs) {
				res.status(404).send({message: 'error en la petición de populate no hay songs' });
			}else{
...
function updateSong(req, res) {
	...
	Song.findByIdAndUpdate(songId, update, (err, songUpdated) =>{
		if(err){
			res.status(500).send({message : 'error al hace update song'});
		}else{
			if (!songUpdated) {
				res.status(404).send({message : 'no se ha actualizado el song'});
			}else{
	...
	if(req.files){
		...
		if (file_ext == 'mp3' || file_ext == 'ogg'){
			Song.findByIdAndUpdate(songId, {file: file_name}, (err, songUpdated) =>{
				if (!songUpdated) {
					res.status(404).send({
						message: 'No se ha podido actualizar el song'
					});	
				}else{
					res.status(200).send({song: songUpdated});
				}
